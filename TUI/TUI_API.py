...
class API():
    ...
    def game_loop(self):
        ...
        while True:
            ...
            print(self.tui.main_menu+self.tui.menu_selection)
            if self.hang.get_user() == "":
                uInput = input("    Veldu aðgerð: ")
            else:
                uInput = input("    ({}) {}: ".format(
                    self.hang.max_wrong_guesses, self.hang.get_user()))
            if uInput == "p" or uInput == "P":
                ...
                # Resets maximum guesses to default on new user (10)
                self.hang.set_max_wrong_guesses()
            elif uInput == "g" or uInput == "G":
                print("\n" * API.NUMBER_OF_EMPTY_LINES)
                print(self.tui.guess_text)
                max_guesses = input(
                    "    Enter new maximum guesses (default 10): ")
                try:
                ...
                print("\n" * API.NUMBER_OF_EMPTY_LINES)
                print(self.tui.wrong_selection)
                time.sleep(2)
